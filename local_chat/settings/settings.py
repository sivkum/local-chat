...
from local_chat.settings.settings_loader import load_active_settings
...
class ServerSettings(BaseModel):
    ...
    port: int = Field(description="Port of LocalChat FastAPI server, defaults to 8001")
...
class ClickHouseSettings(BaseModel):
    ...
    client_cert_key: str | None = Field(
        None,
        description="Path to the local key for the TLS Client certificate",
    )
...
class MilvusSettings(BaseModel):
    uri: str = Field(
        "local_data/local_chat/milvus/milvus_local.db",
        description="The URI of the Milvus instance. For example: 'local_data/local_chat/milvus/milvus_local.db' for Milvus Lite.",
    )
    token: str = Field(
        "",
        description=(
