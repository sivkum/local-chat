...
test-coverage:
	PYTHONPATH=. poetry run pytest tests --cov local_chat --cov-report term --cov-report=html --cov-report xml --junit-xml=tests-results.xml

black:
	poetry run black . --check

ruff:
	poetry run ruff check local_chat tests

format:
	poetry run black .
	poetry run ruff check local_chat tests --fix

mypy:
	poetry run mypy local_chat
...
dev-windows:
	(set PGPT_PROFILES=local & poetry run python -m uvicorn local_chat.main:app --reload --port 8001)

dev:
	PYTHONUNBUFFERED=1 PGPT_PROFILES=local poetry run python -m uvicorn local_chat.main:app --reload --port 8001
...
api-docs:
	PGPT_PROFILES=mock poetry run python scripts/extract_openapi.py local_chat.main:app --out fern/openapi/openapi.json
